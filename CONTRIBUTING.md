...
Optional but recommended: `pip install pre-commit && pre-commit install --hook-type pre-commit --hook-type pre-push` runs the index regeneration, the version bump check (against your local `origin/main`) and the tooling tests before each commit, so CI rarely surprises you.
