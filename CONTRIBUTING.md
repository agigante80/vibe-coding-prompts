...
Optional but recommended: `pip install pre-commit && pre-commit install` sets up two layers of local checks: index regeneration and tooling tests run at commit time, and the version bump check runs at push time against the refs you are pushing. Together they mean CI rarely surprises you.
